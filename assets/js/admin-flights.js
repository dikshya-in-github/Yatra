/* =========================================
   ...
   Mock CRUD (Master Plan §2.1 #4): data lives in
   localStorage until the Spring API replaces it
   (dynamic-readiness rule — swap the crud layer
   for api.js calls, keep the same data shapes).

   Key rules baked in:
   - The airline dropdown is fed from the AIRLINES
     store (yatra_admin_airlines) — flights always
     reference an airline by id, never free text.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Airlines are OWNED by admin-airlines.js — this page only reads them.
     Falls back to the 4 built-in carriers if that store is empty. */
