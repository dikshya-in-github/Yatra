/* =====================================================
   MOCK ESEWA GATEWAY — amount from selected flight,
   eye toggle, fake captcha, simulated login → e-ticket
   ===================================================== */
document.addEventListener("DOMContentLoaded", function () {
    ...
    /* ---------- Load selected flight (fallback = demo values) ---------- */
    var flight = null;
    try {
        flight = JSON.parse(sessionStorage.getItem("yatra_selected_flight"));
    } catch (e) { /* ignore */ }

    var amount = flight && flight.price ? flight.price : 8299.99;
    ...
    form.addEventListener("submit", function (e) {
        e.preventDefault();
        formErr.textContent = "";
        setIdErr(""); setPinErr(""); setCapErr("");

        var id = document.getElementById("esewaId").value.trim();
