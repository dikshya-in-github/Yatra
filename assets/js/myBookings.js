/* =========================================
   ...
   (checklist item 14). Reads the SAME shared
   store the admin panel reads:
     localStorage `yatra_bookings`
   written by esewaConfirm.js on PAY success
   and seeded by admin-bookings.js — so what
   the customer sees here is exactly what the
   admin sees in admin-bookings.html.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const BOOKING_KEY = 'yatra_bookings'; // shared with the admin panel
  const PAGE_SIZE = 6;
  ...
  $('#dEticketBtn').addEventListener('click', () => {
    const b = readBookings().find((x) => x.id === currentBkgId);
    if (!b) return;
    const pay = b.payment || {};
  ...
  $('#bookingList').addEventListener('click', (e) => {
    ...
    if (viewBtn) {
      const b = readBookings().find((x) => x.id === viewBtn.dataset.view);
      if (b) openDetail(b);
    }
  });
